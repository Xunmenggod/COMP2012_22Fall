...
num_credit(num_credit), capacity(course_capacity), size(0)
{
    // TODO
...
:num_credit(course.num_credit), capacity(course.capacity), size(course.size)
{
    // TODO
    name = new char[strlen(course.name) + 1];
    strcpy(name, course.name);

    wait_list = new Wait_List(*course.wait_list);
